...
end=[date(2016,4,11),date(2016,4,10)]
for enddays in end:
    x=enddays.day
    # 取前10天交互的user-item对
    select_user_item = action8[(action8.date >= enddays - timedelta(14)) & (action8.date <= enddays - timedelta(5))]
    ...
    for i in (1, 2, 3, 5, 6):
        ...
        train_temp = pd.merge(train_temp, df[['user_id', 'all_before61_user_type4_rate_type%s' % (i)]], on='user_id',
                              how='left')
    train = pd.merge(train, train_temp, on='user_id', how='left')
    ...
    for i in (1, 2, 3, 5, 6):
        ...
        train_temp = pd.merge(train_temp, df[['user_id', 'all_before56_user_type4_rate_type%s' % (i)]], on='user_id',
                              how='left')
    train = pd.merge(train, train_temp, on='user_id', how='left')
    ...
    for i in (1, 2, 3, 5, 6):
        ...
        train_temp = pd.merge(train_temp, df[['user_id', 'all_before49_user_type4_rate_type%s' % (i)]], on='user_id',
                              how='left')
    train = pd.merge(train, train_temp, on='user_id', how='left')
    ...
    for i in (1, 2, 3, 5, 6):
        ...
        train_temp = pd.merge(train_temp, df[['user_id', 'all_before42_user_type4_rate_type%s' % (i)]], on='user_id',
                              how='left')
    train = pd.merge(train, train_temp, on='user_id', how='left')
    ...
    for i in (1, 2, 3, 5, 6):
        ...
        train_temp = pd.merge(train_temp, df[['user_id', 'all_before35_user_type4_rate_type%s' % (i)]], on='user_id',
                              how='left')
    train = pd.merge(train, train_temp, on='user_id', how='left')
    ...
    for i in (1, 2, 3, 5, 6):
        ...
        train_temp = pd.merge(train_temp, df[['user_id', 'all_before28_user_type4_rate_type%s' % (i)]], on='user_id',
                              how='left')
    train = pd.merge(train, train_temp, on='user_id', how='left')
    ...
    for i in (1, 2, 3, 5, 6):
        ...
        train_temp = pd.merge(train_temp, df[['user_id', 'all_before21_user_type4_rate_type%s' % (i)]], on='user_id',
                              how='left')
    train = pd.merge(train, train_temp, on='user_id', how='left')
    ...
    for i in (1, 2, 3, 5, 6):
        ...
        train_temp = pd.merge(train_temp, df[['user_id', 'all_before14_user_type4_rate_type%s' % (i)]], on='user_id',
                              how='left')
    train = pd.merge(train, train_temp, on='user_id', how='left')
    ...
    for i in (1, 2, 3, 5, 6):
        ...
        train_temp = pd.merge(train_temp, df[['user_id', 'all_before7_user_type4_rate_type%s' % (i)]], on='user_id',
                              how='left')
    train = pd.merge(train, train_temp, on='user_id', how='left')
    ...
    # 前14
    user_all_before14_cate8_action = actionAll[actionAll.time_diff <= 14][['user_id', 'type', 'cate']]
    ...
    temp = df.groupby('user_id')[['cate', 'type_1']].apply(user_cate8_type1_rate_all).reset_index()
    temp.columns = ['user_id', 'user_cate8_type1_rate_all_before14']
    train = pd.merge(train, temp, on='user_id', how='left')
    ...
    temp = df.groupby('user_id')[['cate', 'type_2']].apply(user_cate8_type2_rate_all).reset_index()
    temp.columns = ['user_id', 'user_cate8_type2_rate_all_before14']
    train = pd.merge(train, temp, on='user_id', how='left')
    ...
    temp = df.groupby('user_id')[['cate', 'type_3']].apply(user_cate8_type3_rate_all).reset_index()
    temp.columns = ['user_id', 'user_cate8_type3_rate_all_before14']
    train = pd.merge(train, temp, on='user_id', how='left')
    ...
    temp = df.groupby('user_id')[['cate', 'type_4']].apply(user_cate8_type4_rate_all).reset_index()
    temp.columns = ['user_id', 'user_cate8_type4_rate_all_before14']
    train = pd.merge(train, temp, on='user_id', how='left')
    ...
    temp = df.groupby('user_id')[['cate', 'type_5']].apply(user_cate8_type5_rate_all).reset_index()
    temp.columns = ['user_id', 'user_cate8_type5_rate_all_before14']
    train = pd.merge(train, temp, on='user_id', how='left')
    ...
    temp = df.groupby('user_id')[['cate', 'type_6']].apply(user_cate8_type6_rate_all).reset_index()
    temp.columns = ['user_id', 'user_cate8_type6_rate_all_before14']
    train = pd.merge(train, temp, on='user_id', how='left')
    ...
    user_all_before14_action_days = actionAll[actionAll.time_diff <= 14][['user_id', 'type', 'cate', 'time_diff']]
    temp = user_all_before14_action_days.groupby('user_id')['time_diff'].apply(have_action_days).reset_index()
    temp.columns = ['user_id', 'all_user_before14_action_days']
    train = pd.merge(train, temp, on='user_id', how='left')
    ...
    user_all_action_days = actionAll[['user_id', 'type', 'cate', 'time_diff']]
    temp = user_all_action_days.groupby('user_id')['cate', 'time_diff'].apply(
        user_cate8_action_days_rate_all).reset_index()
    ...
    user_all_before21_action_days = actionAll[actionAll.time_diff <= 21][['user_id', 'type', 'cate', 'time_diff']]
    temp = user_all_before21_action_days.groupby('user_id')['cate', 'time_diff'].apply(
        user_cate8_action_days_rate_all).reset_index()
    temp.columns = ['user_id', 'user_cate8_before21_action_days_rate_all']
    train = pd.merge(train, temp, on='user_id', how='left')

    user_all_before14_action_days = actionAll[actionAll.time_diff <= 14][['user_id', 'type', 'cate', 'time_diff']]
    temp = user_all_before14_action_days.groupby('user_id')['cate', 'time_diff'].apply(
        user_cate8_action_days_rate_all).reset_index()
    temp.columns = ['user_id', 'user_cate8_before14_action_days_rate_all']
    train = pd.merge(train, temp, on='user_id', how='left')

    user_all_before7_action_days = actionAll[actionAll.time_diff <= 7][['user_id', 'type', 'cate', 'time_diff']]
    temp = user_all_before7_action_days.groupby('user_id')['cate', 'time_diff'].apply(
        user_cate8_action_days_rate_all).reset_index()
    ...
    temp = user_buy_action_days.groupby('user_id')['type', 'time_diff'].apply(
        user_buy_action_days_rate_all).reset_index()
    ...
    temp = user_before21_buy_action_days.groupby('user_id')['type', 'time_diff'].apply(
        user_buy_action_days_rate_all).reset_index()
    temp.columns = ['user_id', 'user_before21_buy_action_days_rate']
    train = pd.merge(train, temp, on='user_id', how='left')

    user_before14_buy_action_days = actionAll[actionAll.time_diff <= 14][['user_id', 'type', 'time_diff']]
    temp = user_before14_buy_action_days.groupby('user_id')['type', 'time_diff'].apply(
        user_buy_action_days_rate_all).reset_index()
    temp.columns = ['user_id', 'user_before14_buy_action_days_rate']
    train = pd.merge(train, temp, on='user_id', how='left')

    user_before7_buy_action_days = actionAll[actionAll.time_diff <= 7][['user_id', 'type', 'time_diff']]
    temp = user_before7_buy_action_days.groupby('user_id')['type', 'time_diff'].apply(
        user_buy_action_days_rate_all).reset_index()
    ...
    temp = user_buy_action_days[user_buy_action_days.type == 4].groupby('user_id')['time_diff'].apply(
        user_action_days_diff_max).reset_index()
    temp.columns = ['user_id', 'user_two_buy_days_diff_max']
    train = pd.merge(train, temp, on='user_id', how='left')

    temp = user_buy_action_days[user_buy_action_days.type == 4].groupby('user_id')['time_diff'].apply(
        user_action_days_diff_min).reset_index()
    temp.columns = ['user_id', 'user_two_buy_days_diff_min']
    train = pd.merge(train, temp, on='user_id', how='left')

    temp = user_buy_action_days[user_buy_action_days.type == 4].groupby('user_id')['time_diff'].apply(
        user_action_days_diff_mean).reset_index()
    temp.columns = ['user_id', 'user_two_buy_days_diff_mean']
    train = pd.merge(train, temp, on='user_id', how='left')

    temp = user_buy_action_days[user_buy_action_days.type == 4].groupby('user_id')['time_diff'].apply(
        user_action_days_diff_std).reset_index()
    temp.columns = ['user_id', 'user_two_buy_days_diff_std']
    train = pd.merge(train, temp, on='user_id', how='left')

    train.fillna(0, inplace=True)
    train.to_csv('./sub/train_all_cate_action_feature_%s.csv'%x, index=False)
