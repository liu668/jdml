...
actionAll=pd.read_csv(u'/home/javis/jd2017/jdata/JData_actionAll_drop_duplicates.csv')
actionAll['date']=pd.to_datetime(actionAll['date'],format='%Y/%m/%d')
...
#取前10天交互的user-item对
select_user_item=action8[(action8.date>=enddays-timedelta(29)) & (action8.date<=enddays)]
train=select_user_item[['user_id']].drop_duplicates()

#特征提取
actionAll=actionAll[ (actionAll.date<=enddays)]
actionAll['time_diff'] = [-int(i.days) for i in (actionAll['date'] - (enddays+timedelta(1)))]
...
train.to_csv('./sub/test_all_cate_action_feature_15.csv',index=False)
